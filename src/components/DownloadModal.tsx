...
export const DownloadModal: React.FC<DownloadModalProps> = ({ isOpen, onClose, config }) => {
  ...
  const files = [
    ...
    {
      name: 'install.sh',
      content: `#!/usr/bin/env bash\n# Mindwtr Proxmox VE LXC One-Line Installer\n# Repo: https://github.com/${config.githubUser || 'dongdongbh'}/${config.githubRepo || 'Mindwtr'}\nbash -c "$(curl -fsSL https://raw.githubusercontent.com/${config.githubUser || 'dongdongbh'}/${config.githubRepo || 'Mindwtr'}/${config.branch || 'main'}/ct/mindwtr.sh)"\n`,
      filename: 'install.sh',
      desc: 'Repository root quick launcher',
      icon: FileCode,
    },
    {
